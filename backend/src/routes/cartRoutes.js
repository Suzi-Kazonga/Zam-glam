  ...
  try {
    const [rows] = await pool.query(
      `SELECT c.product_id AS id, c.quantity, p.name, p.price, p.image_url, p.stock
       FROM cart c JOIN customers customer ON customer.id = c.customer_id
       JOIN products p ON p.id = c.product_id WHERE customer.user_id = ?`,
      [req.user.id],
    );
    res.json(rows);
...
router.post('/', authMiddleware, async (req, res, next) => {
  try {
    ...
    if (!customers[0]) return res.status(404).json({ error: 'Customer profile not found' });
    await pool.query(
      `INSERT INTO cart (customer_id, product_id, quantity) VALUES (?, ?, ?)
       ON DUPLICATE KEY UPDATE quantity = quantity + VALUES(quantity)`,
      [customers[0].id, product_id, quantity],
    );
