...
export default function Topbar({ onSearch }) {
  ...
  return <header className="flex flex-wrap items-center gap-4 border-b border-slate-100 bg-white p-4 shadow">
    <Link to="/" className="text-xl font-bold tracking-tight text-slate-900 transition hover:text-indigo-600">Zamglam</Link>
    <div className="relative ml-auto flex items-center gap-2">
      <CartIcon />
      <button onClick={() => setMenuOpen(!menuOpen)} className="flex items-center gap-2 rounded-lg p-1 transition hover:bg-slate-100"><span className="flex h-9 w-9 items-center justify-center rounded-full bg-indigo-100 font-bold text-indigo-700">{user?.name?.charAt(0)?.toUpperCase() || 'U'}</span><span className="hidden text-sm font-semibold text-slate-700 sm:block">{user?.name || 'Account'}</span></button>
