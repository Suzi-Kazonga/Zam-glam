...
function LoginPage() {
  const [email, setEmail] = useState(CUSTOMER_CREDENTIALS.email);
  ...
  const continueAfterLogin = (nextUser) => {
    navigate(getPostLoginPath(nextUser, from, getTotalItems()), { replace: true });
  };
  ...
  const handleRoleChange = (nextRole) => {
    ...
    if (nextRole === 'admin') {
      ...
      setEmail(CUSTOMER_CREDENTIALS.email);
      setPassword('');
    } else if (nextRole === 'seller') {
      setEmail(SELLER_CREDENTIALS.email);
      setPassword('');
    } else if ([ADMIN_CREDENTIALS.email, CUSTOMER_CREDENTIALS.email, SELLER_CREDENTIALS.email].includes(email)) {
      setEmail('');
    }
  };
